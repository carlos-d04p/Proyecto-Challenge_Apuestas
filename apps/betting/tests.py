...
from apps.betting.services import place_simple_bet, settle_bet, place_acca_bet, cash_out_bet, settle_acca_bet
...
class TestPlaceSimpleBet:
    ...
    def test_stake_minimo_10_rechazado(self, test_user, active_selection):
    ...
    def test_limite_dinamico_20000_rechazado(self, test_user, active_selection):
        stake_excesivo = Decimal("10000.0000")
        with pytest.raises(ValidationError, match="stake máximo"):
            place_simple_bet(
                user=test_user,
                selection_id=active_selection.id,
                stake=stake_excesivo,
                expected_odds=Decimal("2.1000")
            )
...
class TestPlaceAccaBet:
    ...
    def test_acca_mas_de_5_selecciones_rechazada(self, test_user, db):
        selections = []
        for i in range(6):
            event = Event.objects.create(
                name=f"Evento {i}", sport="Fútbol",
                starts_at=timezone.now() + timedelta(days=i + 1),
                status=Event.Status.SCHEDULED
            )
            market = Market.objects.create(event=event, kind=Market.Kind.MATCH_RESULT, name="1X2")
            sel = Selection.objects.create(market=market, name="Local", odds=Decimal("2.0000"))
            selections.append(sel.id)

        with pytest.raises(ValidationError, match="máximo de 5 selecciones"):
            place_acca_bet(
                user=test_user,
                selection_ids=selections,
                stake=Decimal("10.0000"),
                expected_odds=Decimal("64.0000")
            )


class TestSettleAccaBet:
    def test_void_parcial_recalcula_cuota(self, test_user, active_selection, second_selection):
        bet = Bet.objects.create(
            user=test_user,
            stake=Decimal("10.0000"),
            total_odds=Decimal("4.2000"),
            status=Bet.Status.PLACED,
            bet_type=Bet.Type.ACCA
        )
        bs1 = BetSelection.objects.create(bet=bet, selection=active_selection, odds_at_placement=Decimal("2.1000"))
        bs2 = BetSelection.objects.create(bet=bet, selection=second_selection, odds_at_placement=Decimal("2.0000"))

        result = settle_acca_bet(bet, {
            str(bs1.id): BetSelection.Result.VOID,
            str(bs2.id): BetSelection.Result.WON,
        })
        assert result.status == Bet.Status.WON
        assert result.payout == Decimal("20.0000")

    def test_lost_en_una_seleccion_pierde_todo(self, test_user, active_selection, second_selection):
        bet = Bet.objects.create(
            user=test_user,
            stake=Decimal("10.0000"),
            total_odds=Decimal("4.2000"),
        ...
        result = settle_acca_bet(bet, {
            str(bs1.id): BetSelection.Result.LOST,
            str(bs2.id): BetSelection.Result.WON,
        })
        assert result.status == Bet.Status.LOST
...
class TestCashOutBet:
    ...
    def test_cashout_rechazado_cuota_menor_a_1(self, placed_bet):
        with pytest.raises(ValidationError, match="menor a 1.0000"):
            cash_out_bet(placed_bet, current_odds=Decimal("0.9000"))

    def test_cashout_bloqueado_10_min_antes(self, test_user, db):
        event_inmediato = Event.objects.create(
            name="Partido Inminente", sport="Fútbol",
            starts_at=timezone.now() + timedelta(minutes=5),
            status=Event.Status.SCHEDULED
        )
        market = Market.objects.create(event=event_inmediato, kind=Market.Kind.MATCH_RESULT, name="1X2")
        sel = Selection.objects.create(market=market, name="Local", odds=Decimal("2.0000"))

        bet = Bet.objects.create(
            user=test_user,
            stake=Decimal("10.0000"),
            total_odds=Decimal("2.0000"),
            status=Bet.Status.PLACED,
            bet_type=Bet.Type.SINGLE
        )
        BetSelection.objects.create(bet=bet, selection=sel, odds_at_placement=Decimal("2.0000"))

        with pytest.raises(ValidationError, match="menos de 10 minutos"):
            cash_out_bet(bet, current_odds=Decimal("2.0000"))